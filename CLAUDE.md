...
- **Products source of truth**: the wholesale catalogue and prices live in the Fudo menu `https://menu.fu.do/independencia180/qr-menu` (store name "Honesto Obrador"). The site lists categories and products with descriptions but **never prices** (they change too often); link to Fudo for prices. When the menu changes, update `bakery.html` (cards, FAQ, OfferCatalog), the home `hasMenu`, and `llms.txt` together.
